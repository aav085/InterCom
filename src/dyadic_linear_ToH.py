...
class Dyadic_Linear_ToH(Temporal_Overlapped_DWT):
    ...
    def _load_custom_toh(self) -> List[float] | None:
        ...
        try:
            ...
            logging.warning("Error reading %s: %s", table_path, exc)
            return None

        # Validación de longitud (para dwt_levels=6, esperamos 7 valores)
        expected = 1 + self.dwt_levels
        if len(values) != expected:
            logging.warning("Mismatch: loaded %d values, expected %d.", len(values), expected)
            return None
